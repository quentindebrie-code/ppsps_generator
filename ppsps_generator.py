...
def _wrap_title_for_header(title, available_width):
    ...
    def _best_split(text, max_w, size):
        words = text.split()
        if len(words) < 2:
            return None
    ...
    for size in [8.0, 7.5, 7.0, 6.5, 6.0]:
        if stringWidth(title, FONT, size) <= available_width:
            return [title], size
        split = _best_split(title, available_width, size)
        if split:
...
class ColorRect(Flowable):
    def __init__(self, width, height, fill_color):
        super().__init__()
        self.width = width; self.height = height; self.fill = fill_color
    ...
    def _draw_header(canvas):
        ...
        if title_lines:
            ...
            if len(title_lines) == 1:
                ...
                canvas.drawRightString(rx, y_top,                 title_lines[0])
                canvas.drawRightString(rx, y_top - title_leading, title_lines[1])
...
def _section_risques(story, data):
    ...
    story.append(risk_table); story.append(PageBreak())
