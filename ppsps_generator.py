...
Modifications v2 :
  - Logo SVG Deldossi Assainissement intégré (page de couverture + en-têtes)
...
# Hauteur de la bande d'en-tête (augmentée pour 2 lignes de titre + logo)
HEADER_H = 40   # était 28

# ── Ratio naturel du logo SVG (viewBox 1320 x 274) ───────────────────────────
_LOGO_RATIO = 1320 / 274   # ≈ 4.817

# ═══════════════════════════════════════════════════════════════════════════════
# LOGO — chargement et mise à l'échelle
# ═══════════════════════════════════════════════════════════════════════════════

def _scale_drawing(drawing, target_height):
    """Redimensionne un Drawing svglib à la hauteur cible (en points)."""
    if drawing is None:
        return None
...
def _load_logo(logo_path, target_height):
    """
    Charge le SVG logo et retourne un Drawing à la hauteur demandée.
    Retourne None si svglib n'est pas disponible ou si le fichier est absent.
    """
    if not logo_path or not HAS_SVGLIB:
        return None
    if not os.path.isfile(logo_path):
        return None
    try:
        drw = svg2rlg(logo_path)
        return _scale_drawing(drw, target_height)
    except Exception:
        return None
...
def _wrap_title_for_header(title, available_width):
    """
    Retourne (lignes: list[str], font_size: float).
    Essaie de faire tenir le titre en 1 ligne, puis 2, en réduisant
    progressivement la taille de la police de 8 à 6 pt.
    En dernier recours, tronque avec '…'.
    """
    FONT  = "Helvetica"
    ...
    def _best_two_line_split(text, max_w, size):
        words = text.split()
        if len(words) < 2:
            return None
        ...
        for i in range(1, len(words)):
            l1 = " ".join(words[:i])
            l2 = " ".join(words[i:])
            w1 = stringWidth(l1, FONT, size)
            w2 = stringWidth(l2, FONT, size)
    ...
    for size in [8.0, 7.5, 7.0, 6.5, 6.0]:
        if stringWidth(title, FONT, size) <= available_width:
            return [title], size
        split = _best_two_line_split(title, available_width, size)
        if split:
            return list(split), size

    # Dernier recours : tronquer mot par mot
    words = title.split()
    for size in [6.0]:
        for n in range(len(words), 0, -1):
            ...
            if n < len(words):
                candidate += "…"
            if stringWidth(candidate, FONT, size) <= available_width:
...
def _make_page_template(entreprise_nom, projet_intitule, logo_path=None):
    """
    Retourne (on_first_page, on_page).
    En-tête de HEADER_H pt avec logo (ou nom entreprise en fallback) à gauche
    et titre du chantier (max 2 lignes, taille adaptative) à droite.
    """
    # Pré-calcul du logo header (une seule fois, réutilisé sur chaque page)
    logo_h        = HEADER_H - 12          # marge 6 pt haut et bas → 28 pt
    logo_drawing  = _load_logo(logo_path, target_height=logo_h)
    logo_w        = logo_drawing.width if logo_drawing else 0

    # Largeur disponible pour le titre (côté droit)
    if logo_drawing:
        left_used = ML + logo_w + 12
    else:
        ent_w     = stringWidth(entreprise_nom or "", "Helvetica-Bold", 9)
        left_used = ML + ent_w + 15
    title_avail_w = (PW - MR) - left_used

    # Pré-calcul du wrap (idem pour toutes les pages)
    title_lines, title_fs = _wrap_title_for_header(projet_intitule, title_avail_w)
    title_leading = title_fs * 1.4

    def _draw_header(canvas):
        """Dessine la bande haute avec logo + titre."""
        canvas.setFillColor(NAVY)
        canvas.rect(0, PH - HEADER_H, PW, HEADER_H, fill=1, stroke=0)
        canvas.setFillColor(WHITE)

        # ── Logo ou nom entreprise (gauche) ──────────────────────────────────
        if logo_drawing and HAS_SVGLIB:
            logo_y = PH - HEADER_H + (HEADER_H - logo_h) / 2
            renderPDF.draw(logo_drawing, canvas, ML, logo_y)
        else:
            canvas.setFont("Helvetica-Bold", 9)
            canvas.drawString(ML, PH - HEADER_H / 2 - 4, entreprise_nom or "")

        # ── Titre chantier (droite, max 2 lignes) ────────────────────────────
        if title_lines:
            canvas.setFillColor(WHITE)
            ...
            n = len(title_lines)
            if n == 1:
                # Centré verticalement dans la bande
                y = PH - HEADER_H / 2 - title_fs / 2 + 1
                canvas.drawRightString(right_x, y, title_lines[0])
            else:
                # Bloc 2 lignes centré verticalement
                block_h = title_fs + title_leading
                y_top   = PH - HEADER_H / 2 + block_h / 2 - title_fs
                canvas.drawRightString(right_x, y_top,              title_lines[0])
                canvas.drawRightString(right_x, y_top - title_leading, title_lines[1])
    ...
    def on_page(canvas, doc):
        ...
        canvas.drawRightString(PW - MR, 6, f"Page {doc.page}")

        canvas.restoreState()

    def on_first_page(canvas, doc):
        canvas.saveState()
        # Pied de page simple sur la couverture (pas d'en-tête sur la couverture)
        canvas.setFillColor(NAVY)
        canvas.rect(0, 0, PW, 20, fill=1, stroke=0)
        canvas.setFillColor(WHITE)
        canvas.setFont("Helvetica", 7)
...
def _info_table(rows, col_w=None):
    ...
    data = [
        [P(label, "TLabel"), P(val or "—", "TVal") if isinstance(val, str) else val]
...
def _chk(checked, size=7):
    """Carré vectoriel : noir plein si coché, blanc avec contour sinon."""
...
def _page_cover(story, data, logo_path=None):
    ...
    # ── Logo ou nom entreprise en haut de la couverture ──────────────────────
    cover_logo = _load_logo(logo_path, target_height=45)
    if cover_logo:
    ...
    meta_rows = []
    if proj.get("situation"):   meta_rows.append(("Situation :",     proj["situation"]))
...
def _section_gestion(story, data):
    ...
    suivis = gestion.get("suivis", [
        {"indice":"A","date":proj.get("date_creation",""),"nature":"Création du document"}
    ])
    suivi_data = [[P("Indice","TH"), P("Date","TH"), P("Nature de révision","TH")]]
    for s in suivis:
        suivi_data.append([P(s.get("indice",""),"TD_C"),
                           P(s.get("date",""),"TD_C"),
                           P(s.get("nature",""),"TD")])
...
def _section_prevention(story, data):
    ...
    story.append(SP(4))
    story.append(P(
        "Afin de sensibiliser le personnel à sa présence sur site, les consignes réglementaires "
    ...
    story.append(SP(4))
    story.append(P(
        "Sur ce chantier, toutes les personnes intervenantes doivent disposer des EPI suivants :",
        "Body"
    ))
    for e in epi_list:
...
def generer_ppsps(data: dict, logo_path: str = None) -> io.BytesIO:
    ...
    Paramètres
    ----------
    data      : dict  — données du formulaire
    logo_path : str   — chemin vers le fichier SVG du logo (optionnel).
                        Si absent ou si svglib n'est pas installé, le nom
                        de l'entreprise est utilisé en mode texte.
    """
    buf = io.BytesIO()
    ent  = data.get("entreprise", {})
    proj = data.get("projet", {})
    ...
    doc = SimpleDocTemplate(
        ...
        topMargin=MT + HEADER_H,   # ajusté à la nouvelle hauteur d'en-tête
        bottomMargin=MB + 20,
